...
NSString *const fitpolo705AdditionalInformation = @"fitpolo705AdditionalInformation";
NSString *const fitpolo705DataInformation = @"fitpolo705DataInformation";
NSString *const fitpolo705DataStatusLev = @"fitpolo705DataStatusLev";

@interface fitpolo705TaskOperation()
...
@implementation fitpolo705TaskOperation
...
- (void)peripheral:(CBPeripheral *)peripheral didUpdateValueForCharacteristic:(CBCharacteristic *)characteristic error:(NSError *)error
{
    ...
    NSDictionary *dic = [fitpolo705DataParser parseReadDataFromCharacteristic:characteristic];
    if (!fitpolo705ValidDict(dic)) {
        return;
    }
    ...
    if (self.completeBlock) {
        if (self.needPartOfData) {
            ...
            NSDictionary *resultDic = @{
                                        fitpolo705AdditionalInformation:(self.additionalInformation ?: @{}),
                                        fitpolo705DataInformation:self.dataList,
                                        //对于有附加信息的，lev为2，对于普通不包含附加信息的，lev为1.
                                        fitpolo705DataStatusLev:(self.additionalInformation ? @"2" : @"1"),
                                        };
            if (self.completeBlock) {
...
- (void)setRespondCount:(NSString *)respondNumber{
    if (!fitpolo705ValidStr(respondNumber)) {
        return;
    }
...
- (void)dataParserReceivedData:(NSDictionary *)dataDic{
    if (self.isCancelled || !_executing || !fitpolo705ValidDict(dataDic)) {
        return;
    }
    fitpolo705TaskOperationID operationID = [dataDic[@"operationID"] integerValue];
    if (operationID == fitpolo705DefaultTaskOperationID) {
        return;
    }
    NSDictionary *returnData = dataDic[@"returnData"];
    if (self.timeout
        || !fitpolo705ValidDict(returnData)
    ...
    NSString *numString = returnData[fitpolo705CommunicationDataNum];
    if (fitpolo705ValidStr(numString)) {
        //本条数据是总数信息
